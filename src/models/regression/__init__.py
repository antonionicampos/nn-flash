...
class NeuralNet(tf.keras.Model):
    def __init__(self, hidden_units, activation, *args, **kwargs):
        ...
        self.outputs = tf.keras.layers.Dense(25, name="output")
...
class ResidualBlock(tf.keras.Model):
    def __init__(self, units, *args, **kwargs):
        ...
        self.units = units
        self.hidden1 = tf.keras.layers.Dense(units, activation=tf.keras.activations.swish)
...
class ResidualNeuralNet(tf.keras.Model):
    def __init__(self, hidden_units, activation, *args, **kwargs):
        ...
        self.residuals = [
            ResidualBlock(units, activation=self.activation, name=f"residual{i+1}")
            for i, units in enumerate(self.hidden_units)
        ]
